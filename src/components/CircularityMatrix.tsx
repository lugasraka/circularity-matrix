...
export default function CircularityMatrix({
  ...
  // Quadrants: [access][process] → (col, row)
  // Y-axis: Access — Easy at bottom, Hard at top
  // X-axis: Process — Easy at left, Hard at right
  function getCellCoords(cellId: string): { x: number; y: number } {
    const parts = cellId.split("-");
    const access = parts[0]; // easy or hard
    const process = parts[1]; // easy or hard
    const embedded = parts[2]; // low or high

    const col = process === "easy" ? 0 : 1;
    // Hard access = top (row 0), Easy access = bottom (row 1)
    const row = access === "hard" ? 0 : 1;

    const x = margin.left + col * quadrantWidth;
    // Within each quadrant, "Low embedded value" is on top, "High embedded value" is on bottom
